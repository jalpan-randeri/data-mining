...
public class Question3 {
    ...
    public static void main(String[] args) {
        ...
        if(args.length != 2){
            System.out.println("Usage Question3 <input_file.csv> <test_file.csv>");
            System.exit(-1);
        }else{
            ...
            try {
                ...
                int lambda = getLambdaUsingCrossValidation(input, k);
                Matrix w = L2RegularizedLinearRegression.calculate_Weights(input, lambda);
                double Ein = L2RegularizedLinearRegression.getMeanSquareError(input.mat_x, input.mat_y, w);
                double Eout = L2RegularizedLinearRegression.getMeanSquareError(test.mat_x, test.mat_y, w);

                System.out.printf("Optimal Lambda is %d Training Error %.4f Testing Error %.4f\n",lambda, Ein, Eout);
    ...
    /**
     * Find the best Lambda Value for the given data set
     *
     * @param input Question representing the int Matrix X, Y
     * @param folds Integer representing the total number of folds
     * @return Integer representing the lambda value
     * @throws IOException
     */
    public static int getLambdaUsingCrossValidation(Question input, int folds) throws IOException {


        InputTestSet set[] = partiation_set(input, folds);
     ...
     * @param index Integer representing the current matrix portion
     * @param length Integer representing the size of one partition in split matrix;
     * @return
     */
